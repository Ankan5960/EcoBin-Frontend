export function calculateEnvironmentalImpact(totalCO2InDustbins: number): { co2EmissionSaved: number; treesSaved: number; } {
  if (typeof totalCO2InDustbins !== 'number' || totalCO2InDustbins < 0) {
    console.error("Invalid input: Total CO2 in dustbins must be a non-negative number.");
