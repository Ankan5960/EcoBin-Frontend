...
const AdminPanel: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<AdminFormInputs> = async (data) => {
    ...
    try {
      const res = await fetch(URLs.AdminSetup, {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        ...
        setResponseMessage(errorResult.error || errorResult.message || `Error: ${res.statusText}`);
  ...
  return (
    <div className="flex-1 p-6 md:p-10 bg-gray-100 min-h-screen"> 
      <div className="max-w-2xl mx-auto">
        ...
        <div className={`${DEFAULT_ITEM_PROPERTIES.heading.heading2} mb-6 text-center md:text-left text-gray-700`}>
        ...
        >
          <div>
            <label htmlFor="dustbinType" className="block text-sm font-medium text-gray-700 mb-1">
              Select Dustbin Type
            </label>
            <select
              id="dustbinType" 
              {...register("dustbinType", { required: "Please select a dustbin type." })} 
        ...
        {responseMessage && (
          <div className="mt-6 bg-white p-4 rounded-lg shadow-md border border-gray-200">
            <div className={`${DEFAULT_ITEM_PROPERTIES.heading.heading3} mb-3 ${
              responseType === 'success' ? 'text-green-700' : 'text-red-700'
            }`}>
              {responseType === 'success' ? 'Generated Dustbin ID' : 'Error'}
            </div>
            <div className={`mt-2 p-3 rounded text-sm flex items-center justify-between gap-4 ${
              responseType === 'success' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'
            }`}>
